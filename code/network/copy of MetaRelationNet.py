...
class RelationNet(torch.nn.Module):
    ...
    def forward(self, support_features, query_features, tensors):
        ...
        """ Merge the support features """
        support_features = torch.bmm(support_features.transpose(1, 2), support_labels_one_hot)
        support_features = support_features.transpose(1, 2).view(batch_size, num_class, num_features)

        unsqueeze_query = query_features.unsqueeze(2).repeat(1, 1, num_class, 1)
        ...
        relations = raw_relations.view(batch_size, num_query * num_class, -1)
        relations = relations.transpose(1, 2)
        relations = self.conv_layers(relations)

        # bs * num_feature_relation * num_query * num_support
        relations = relations.view(batch_size, -1, num_query, num_class)
...
class MetaRelationNet(nn.Module):
    ...
    def forward(self, relations, tensors):
        ...
        num_class = support_labels_one_hot.size(-1)
        num_shot = 1.0 * num_support / num_class
        ...
        out['accuracy_relation'] = accuracy1
        out['accuracy'] = accuracy
