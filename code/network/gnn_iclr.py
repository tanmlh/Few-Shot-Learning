...
class Gconv(nn.Module):
    ...
    def forward(self, input):
        ...
        x = x.view(*(list(x_size[:-1]) + [self.num_outputs]))
        return W, x
...
class Wcompute(nn.Module):
    ...
    def forward(self, x, W_id):
        bs, num_samples, num_features = x.size()
        W1 = x.unsqueeze(2)
        W1 = W1.expand(bs, num_samples, num_samples, num_features)
...
class GNN_nl_omniglot(nn.Module):
    def __init__(self, opt, input_features, nf, J):
        ...
        self.layer_last = Gconv(self.input_features + int(self.nf / 2) * self.num_layers, opt['num_ways'], 2, bn_bool=True)

    def cuda(self, *args):
        super(GNN_nl_omniglot, self).cuda()
        self.W_init.cuda()

    def forward(self, x, tensors):
        W_init = tensors['W_init'].resize_(x.size(0), x.size(1), x.size(1), 1).copy_(
...
class GNN_nl(nn.Module):
    def __init__(self, opt, input_features, nf, J):
        ...
        self.layer_last = Gconv(self.input_features + int(self.nf / 2) * self.num_layers,
                                opt['num_ways'], 2, bn_bool=False)
