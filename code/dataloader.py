...
class MiniImageNet(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        if self.phase=='train':
            ...
            self.data, self.labels = data_train
            ...
            self.data, self.labels = data_novel
            self.label2ind = buildLabelIndex(self.labels)
            self.labelIds = sorted(self.label2ind.keys())
            self.num_cats = len(self.labelIds)
        else:
...
class FewShotDataloader():
    def __init__(self,
        ...
        max_possible_nKnovel = (self.dataset.num_cats if self.phase=='train'
                                else self.dataset.num_cats)
    ...
    def sample_base_and_novel_categories(self, nKnovel):
        if self.is_eval_mode:
            Knovel = sorted(self.sampleCategories(nKnovel))
        else:
            cats_ids = self.sampleCategories(nKnovel)
    ...
    def sample_episode(self):
        ...
        Knovel = self.sample_base_and_novel_categories(nKnovel)
